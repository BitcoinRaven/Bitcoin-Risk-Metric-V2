...
df = quandl.get("BCHAIN/MKPRU", api_key="*****").reset_index()
df = df[df["Value"] > 0]
...
df.update(normalizationhalving(["usdoverMA", "50d/50w", "ossvalues", "Sharpe"]))
df["avg"] = (df["ossvalues"] * 0.25 + df["50d/50w"] * 0.25 + df["usdoverMA"] * 0.25 + df["Sharpe"] * 0.25) # weighted average
